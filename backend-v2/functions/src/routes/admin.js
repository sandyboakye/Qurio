...
// GET /api/admin/messages - List all support messages
router.get('/messages', async (req, res) => {
    try {
        const messages = await db('support_messages as sm')
            .join('users as u', 'sm.user_id', 'u.id')
            .select(
                'sm.id', 'sm.message', 'sm.is_read', 'sm.created_at',
...
router.put('/messages/:id/read', async (req, res) => {
    try {
        await db('support_messages').where({ id: req.params.id }).update({ is_read: true });
...
router.post('/migrate-support', async (req, res) => {
    try {
        ...
        `);
        return res.json({ message: 'Table created successfully (SQL Mode)' });
    } catch (e) {
